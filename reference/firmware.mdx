---
title: "Firmware Flashing for Denon Prime Go"
sidebarTitle: "Firmware"
description: "Flash custom or stock-with-SSH firmware to the Denon Prime Go using fastboot, USB drive, or the Go-based updater CLI tool."
---
...
The cross-platform Go-based updater uses libusb and the FastBoot protocol directly.
...
The Go updater (`go/cmd/updater/`) implements FastBoot over libusb with a GUI progress display.

| Flag | Default | Description |
|:---|:---:|:---|
| `--firmware <path>` | _(required)_ | Path to the firmware `.img` file to flash |
| `--debug <level>` | `4` | Logging level: `-4` = debug, `0` = info, `4` = error only |
...
```bash
# Example: flash with debug logging to file
cd go && go run ./cmd/updater/ \
  --firmware ../PRIMEGO-4.3.4-STOCK-SSH-Update.img \
  --debug -4 \
