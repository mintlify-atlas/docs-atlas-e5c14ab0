...
| Display connector | LVDS-1 (Prime Go) / DSI-1 (Prime 4) |
...
The Prime 4 uses a DSI-1 display connector rather than the LVDS-1 connector on the Prime Go. Both devices share the same Mali-T76x r1p0 GPU, Rockchip RK3288 SoC, and 800×1280 portrait framebuffer, but the DSI-1 controller applies rotation in the opposite direction and introduces a vsync issue.

### Display Hardware

| Property | Prime Go | Prime 4 |
|---|---|---|
| Connector | LVDS-1 | DSI-1 |
| DRM sysfs path | `card0-LVDS-1` | `card0-DSI-1` |
...
All four rotation values (0, 90, 180, 270) were tested on the Prime 4. Only 270 produced the correct orientation. The DSI-1 controller applies rotation in the opposite direction compared to LVDS-1.

### Screen Tearing Fix (Prime 4)

The DSI-1 DRM path handles page-flip timing differently from LVDS-1 and exhibits visible tearing by default. Enable vsync:

```bash
export QT_QPA_EGLFS_SWAPINTERVAL=1
```

This setting is **not** needed on Prime Go (LVDS-1), which syncs correctly without it.
